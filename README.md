...
- **Поиск и Посдчет**: фильтрация транзакций по ключевому слову с использованием регулярных
выражений и Анализ частоты операций по заданным категориям.
...
Подсчет категорий
```
from src.search_and_count import count_categories
# ...
categories = ["Перевод", "Снятие", "Покупка"]
counts = count_categories(data, categories)
# counts = {'Перевод': 40, 'Снятие': 5, ...}

```
Главная функция (main)

Функция main предоставляет интерактивный консольный интерфейс для пользователя, связывая все модули:

1. Выбор источника данных (JSON/CSV/XLSX).
2. Фильтрация по статусу (EXECUTED/CANCELED/PENDING).
3. Опциональная сортировка по дате.
4. Опциональная фильтрация по рублевым транзакциям.
5. Опциональная фильтрация по слову в описании.
6. Вывод отфильтрованного и замаскированного списка транзакций.
