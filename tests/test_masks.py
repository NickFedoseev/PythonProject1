...
def test_get_mask_card_number_valid_from_fixture(valid_card_numbers):
...
def test_get_mask_card_number_invalid_from_fixture(invalid_card_numbers):
...
def test_get_mask_account_valid_from_fixture(valid_account_numbers):
...
def test_get_mask_account_invalid(account_number, expected_error):
    """Проверка обработки некорректных номеров счетов."""
    with pytest.raises(ValueError, match=expected_error):
        get_mask_account(account_number)


def test_get_mask_account_invalid_from_fixture(invalid_account_numbers):
